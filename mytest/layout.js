...
function top_category(){
	...
	// });
	 const delay=500;
	 let mouseTimeout;
		jQuery('#header .top_category>ul').mouseenter(function(e) {
			clearTimeout(mouseTimeout);
		var $this = jQuery(this).addClass('is-opened');
		jQuery('#header').addClass('d2-opened');
		jQuery(this).children('li').addClass('on');
	});

	jQuery('#header .top_category>ul').mouseleave(function(e) {
		const $this =jQuery(this);
		mouseTimeout = setTimeout(function() {
			jQuery('#header').removeClass('d2-opened');
			jQuery('#header .top_category>ul').removeClass('is-opened');
			jQuery('#header .top_category>ul').children('li').removeClass('on');
		}, delay);
